...
export async function getCvCollaborations(minCount = 2): Promise<CvCollaboration[]> {
  ...
  // drama_id -> Set<cv_id>（仅主役）
  // CP合作只在主役组内两两配对计算
  const dramaMainCvs = new Map<string, Set<string>>();
  for (const r of roles) {
    if (r.role_type !== 'main') continue;
    if (!dramaMainCvs.has(r.drama_id)) dramaMainCvs.set(r.drama_id, new Set());
    dramaMainCvs.get(r.drama_id)!.add(r.cv_id);
  }

  // 统计CV对合作次数：同剧内两人都是主役才算一次CP合作
  const pairMap = new Map<string, { cv1: string; cv2: string; dramaIds: Set<string> }>();
  for (const [dramaId, cvSet] of dramaMainCvs) {
