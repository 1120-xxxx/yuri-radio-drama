...
const props = defineProps<{ dramaId: string; dramaTitle?: string }>();
  ...
  <section class="card">
    ...
    <div class="flex flex-wrap items-start gap-6 mb-6">
      <div class="text-center px-6 py-4 rounded-2xl bg-bg-darker border border-border">
        ...
        <div class="text-4xl font-bold bg-gradient-to-r from-brand-400 to-accent-400 bg-clip-text text-transparent tabular-nums">
      ...
      <div class="flex-1 min-w-[260px]">
        ...
        <div v-if="!backendConnected" class="text-xs text-text-faint mt-3">提示：未配置 Supabase，当前为本地演示模式。</div>
      </div>
    </div>
    ...
    <div v-if="comments.length" class="mt-2">
      ...
      <ul class="space-y-2">
        <li v-for="c in comments.slice(0, 20)" :key="c.id" class="py-3 px-4 rounded-2xl bg-bg-darker border border-border/60">
          <div class="flex items-center gap-2 mb-2">
            <span class="text-brand-300 text-sm font-medium">{{ '★'.repeat(c.score) }}<span class="text-text-faint">{{ '★'.repeat(5 - c.score) }}</span></span>
          </div>
