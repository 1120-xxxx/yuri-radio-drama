  ...
  <div>
    ...
    <div class="card mb-6">
      ...
      <div class="flex flex-wrap gap-3">
        <button v-for="pair in topPairs" :key="`${pair.cv1_id}-${pair.cv2_id}`" @click="openDetail(pair)" :class="[
    ...
    <div class="card">
      ...
      <div class="space-y-2">
        <div v-for="(pair, idx) in sorted.slice(0, 20)" :key="`${pair.cv1_id}-${pair.cv2_id}`" @click="openDetail(pair)"
          ...
          <div class="flex-1 min-w-0">
            <div class="font-medium text-text-soft">
              <a :href="`/cvs/${pair.cv1_id}/`" class="hover:text-brand-300 transition-colors" @click.stop>{{
                pair.cv1_name }}</a>
              <span class="text-brand-400 mx-1">×</span>
              <a :href="`/cvs/${pair.cv2_id}/`" class="hover:text-brand-300 transition-colors" @click.stop>{{
                pair.cv2_name }}</a>
            </div>
          </div>
          <div class="text-right">
            <span
              class="text-lg font-bold bg-gradient-to-r from-brand-400 to-accent-400 bg-clip-text text-transparent">{{
              pair.count }}</span>
        ...
        <div class="space-y-2">
          <div v-for="drama in selected.dramas" :key="drama.id"
            ...
            <a :href="`/dramas/${drama.id}/`"
              class="flex-1 font-medium text-text-soft hover:text-brand-300 transition-colors truncate">{{ drama.title
              }}</a>
            <span v-if="drama.platform"
              class="text-xs px-2 py-0.5 rounded-full bg-bg-soft text-text-muted border border-border">{{ drama.platform
              }}</span>
          </div>
        </div>
      </div>
